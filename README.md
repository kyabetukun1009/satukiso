...
- **テーマディレクトリについて**  
  本リポジトリでは可読性およびレビュー性を重視し、  
  自作テーマを `theme/` ディレクトリ名で配置しています。  

  実運用時は、WordPress のテーマ管理上わかりやすいよう  
  `theme/` ディレクトリを `satukiso-theme/` にリネームし、  
  `wp-content/themes/` 配下へ配置して使用する想定です。

- **ドメイン運用に関する補足**  
  将来的な運用を見据え、地域名を含むドメイン  
 （例：satukiso.nagoya）を使用することで、  
  地域検索を意識したSEO施策が行いやすくなると考えられます。

- **参考サイト**  
  名古屋駅アクセスホテル  
  https://nagoyaeki-access-hotel.com/  
  サイト構成や情報整理、導線設計の参考として一部閲覧しました。
